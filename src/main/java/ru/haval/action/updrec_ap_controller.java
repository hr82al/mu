...
public class updrec_ap_controller {
	...
	public void initialize()
	{
		...
		numpm_tsk.setText(pic._pmnum_ap);
		description_tsk.setText(pic._description_ap);
		type_tsk = pic._type_ap;
		shop_tsk.getSelectionModel().select(sclass.parser_str_str(pic._equip_ap, 0));
		group_tsk.getSelectionModel().select(sclass.parser_str_str(pic._equip_ap, 1));
		lm_tsk.getSelectionModel().select(sclass.parser_str_str(pic._equip_ap, 2));
		os_tsk.getSelectionModel().select(sclass.parser_str_str(pic._equip_ap, 3));
		equip_tsk.getSelectionModel().select(sclass.parser_str_str(pic._equip_ap, 4));
		tsk_maker_ap.setText(qr._select_tm(pic._id_ap));
		oft_tsk.getSelectionModel().select(pic._oft_ap);
		...
		if(type_tsk.equals("TSK")) {
			tsk_ap.setDisable(true);
			cm_ap.setVisible(false);
		}
		else if(type_tsk.equals("PM"))
		{
			tsk_ap.setDisable(true);
			tsk_ap.setText("PM");
			cm_ap.setVisible(false);
		}
		else if(type_tsk.equals("CM")){
			tsk_ap.setVisible(false);
			cm_ap.setDisable(true);
		}
