...
public class PartCharDir_Controller {
	
	@FXML
	TableView<Hmmr_PartCharDir_Model> table_part = new TableView<>();
	
	@FXML
	TableColumn<Hmmr_PartCharDir_Model, String> col_id_part, col_part_char, col_part_char_eng;
	...
	public static ObservableList<Hmmr_PartCharDir_Model> _table_update_part = FXCollections.observableArrayList();
	...
	public void initialize()
	{
		...
		col_part_char.setCellValueFactory(CellData -> CellData.getValue().getPart_Char());
		col_part_char_eng.setCellValueFactory(CellData -> CellData.getValue().getPart_Char_Eng());
		
		_table_update_part.addListener(new ListChangeListener<Hmmr_PartCharDir_Model>() {
			@Override
			public void onChanged(Change<? extends Hmmr_PartCharDir_Model> c) {
				table_part.setItems(qr._select_data_partchardir());
	...
	protected void part_add(Stage stage) throws IOException {
		Parent root = FXMLLoader.load(getClass().getResource("Add_Rec_PartCharDir.fxml"));
	...
	{
		qr._update_partchardir_del(str);
		qr._insert_history(conn_connector.USER_ID, apwr_controller.USER_S + " - Удалил запись № = " + str + " в таблице Part Char Dir");
		_table_update_part.addAll(qr._select_data_partchardir());
	}
}
