...
public class Addrec_PartCharDir_Controller {
	...
	public void initialize()
	{
		...
		if(!pc._flag_window)
		{
			title_part_add.setText("Обновление записи Part Char");
			...
			title_part_add.setText("Добавление записи к Part Char");
		...
		btn_add_part.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent arg0) {
				if(pc._flag_window)
				{
					...
					qr._insert_history(conn_connector.USER_ID, pic.USER_S + " - Добавил запись № = " + qr._select_last_id("hmmr_mu_part") + " в таблице Part Char Dir");
				}
				else
				{
					qr._update_field_partchardir(pc._id_part, txt_part_type.getText(), txt_part_type_eng.getText());
					qr._insert_history(conn_connector.USER_ID, pic.USER_S + " - О запись № = " + qr._select_last_id("hmmr_mu_part") + " в таблице Part Char Dir");
				}
